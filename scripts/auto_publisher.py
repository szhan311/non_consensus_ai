...
"""
    ...
    def _generate_fallback_content(self) -> Tuple[str, str]:
        """当RSS获取失败时生成备用内容"""
        import random
        ...
        fallback_topics = [
            {
                'title': 'AI进步的可信度评估框架',
                'hook': '每当新的SOTA结果发布时，社区的第一反应往往是惊叹。但更值得追问的是：这个结果在多大程度上可信？',
                'angle': '论文声称的改进幅度与实际可复现性之间的gap'
            },
            {
                'title': '从第一性原理看模型规模竞赛',
                'hook': '过去五年，模型参数增长了1000倍。但这种增长是否带来了相称的能力提升？',
                'angle': '规模、效率、可控性的三角权衡'
            },
            {
                'title': '评估指标与真实能力的脱节',
                'hook': '我们在排行榜上追逐更高的分数，但这些分数与实际使用体验是否相关？',
                'angle': 'benchmark gaming如何扭曲研究优先级'
            },
            {
                'title': '开源vs闭源：能力差距的临界点',
                'hook': '闭源模型持续领先，但开源社区的追赶速度是否被低估？',
                'angle': '质量差距 vs 可用性差距的区分'
            },
            {
                'title': 'AI安全的生产化困境',
                'hook': '安全研究产生了大量理论框架，但它们在production环境中的落地情况如何？',
                'angle': '学术安全研究与工程实践的断层'
            }
        ]
...
{topic['angle']}是一个被系统性低估的问题。主流叙事倾向于接受表面结果，而忽视了几个关键问题：
